...
The one local sample produced cold profile attempts `1`, warm-first `3`, and warm-reopen `4` (reopen delta `1`). These are fixture observations from two browser checks, not a backend or cache claim. The current slice cannot attribute identity, cache hits, callback application, or backend time.
