...
The one local sample produced cold profile attempts `3`, warm-first `4`, and warm-reopen `4` (reopen delta `0`). These are fixture observations from two browser checks, not a backend or cache claim. The current slice cannot attribute identity, cache hits, callback application, or backend time.
